...
3. **~~WCAG violation~~ → Product philosophy choice.** I initially flagged `maximum-scale=1` as WCAG 2.1 SC 1.4.4 (Resize Text). Per Ryan's correction: Suite operates as an installed PWA intended to feel like a native chat app, and you can't pinch-zoom into arbitrary parts of native chat apps either. The constraint is intentional. **The right a11y path is in-app accessibility settings** (font-size picker, contrast toggle, reduced-motion respect) — Ryan's framing: "I'd sooner offer accessibility settings in app than have a floaty UI that zooms all the time when you try to navigate the app." Also worth verifying the PWA respects OS-level Dynamic Type / Android font scale. Both are separate investigations / feature work — not a viewport-meta change.
...
- [feature, separate PR] **In-app accessibility settings panel** — font-size picker, contrast toggle, reduced-motion respect. Per Ryan's guidance this is the preferred a11y path over pinch-zoom for a native-feel PWA. Own PR, ~150 LOC (settings UI + user preferences schema + CSS variables driven from user prefs).
