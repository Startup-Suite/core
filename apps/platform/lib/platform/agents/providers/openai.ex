defmodule Platform.Agents.Providers.OpenAI do
  @moduledoc """
  OpenAI provider adapter for the Agent Runtime.

  Supports either:

    * raw API keys (`Authorization: Bearer <api_key>`)
    * OAuth tokens stored in `Platform.Vault` (also Bearer auth)

  The adapter currently targets the Chat Completions API, matching the
  Agent Runtime ADR for the initial OpenAI integration.
  """
  ...
  @models [
    %{id: "gpt-5.4", provider: "openai", auth_mode: "oauth"},
    %{id: "gpt-5.4", provider: "openai", auth_mode: "api_key"}
  ]
  ...
  defp do_chat(%{auth_mode: auth_mode} = resolved, messages, opts) do
    ...
    body =
      %{
        ...
        "max_completion_tokens" => Keyword.get(opts, :max_tokens, 2048)
      }
    ...
    case Req.post(req_client(), url: @api_url, json: body, headers: request_headers(resolved)) do
      {:ok, %{status: 200, body: resp_body, headers: headers}} when is_map(resp_body) ->
  ...
  defp default_model(:oauth), do: "gpt-5.4"
  defp default_model(:api_key), do: "gpt-5.4"

  defp models_for_auth_mode(auth_mode) do
    auth_mode = Atom.to_string(auth_mode)
    Enum.filter(@models, &(&1.auth_mode == auth_mode))
  end

  defp detect_auth_mode(token) when is_binary(token) do
    if String.starts_with?(token, "sk-"), do: :api_key, else: :oauth
  end
  ...
  defp normalize_messages(messages, system_prompt) do
    system_messages =
      case normalize_system_prompt(system_prompt) do
        nil -> []
        prompt -> [%{"role" => "system", "content" => prompt}]
      end

    system_messages ++ Enum.map(messages, &normalize_message/1)
  end

  defp normalize_message(%{} = message) do
    %{}
    |> Map.put("role", normalize_role(fetch_value(message, :role) || "user"))
    |> Map.put("content", normalize_content(fetch_value(message, :content)))
    |> maybe_put_map("name", fetch_value(message, :name))
  end
  ...
  defp first_choice(resp_body) do
    resp_body
    |> Map.get("choices", [])
    |> List.first()
    |> Kernel.||(%{})
  ...
  defp extract_message_content(choice) do
    case get_in(choice, ["message", "content"]) do
      ...
      parts when is_list(parts) ->
        Enum.map_join(parts, "", fn
          ...
          %{"type" => "refusal", "refusal" => text} -> text
          %{"text" => text} -> text
